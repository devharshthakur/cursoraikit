...
2. Identify the scope of changes:
   - Which modules/components/packages were affected
...
4. Examples:
   ...
   - `refactor(utils): improve error handling helpers`
...
- Summary table or list of files changed with brief descriptions: Below is an example

  ```markdown
  | File              | Status   | Description                       |
  | ----------------- | -------- | --------------------------------- |
  | src/auth/login.js | Modified | Added user authentication logic   |
  | src/api/users.js  | Added    | New user management API endpoints |
  ```
