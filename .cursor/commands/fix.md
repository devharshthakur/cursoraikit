...
1. **Context Gathering**: Check for AGENTS.md first, then gather error context
...
5. **Update Documentation**: Update AGENTS.md if agent state changes
...
1. **Check for AGENTS.md**:
   - First, check if `AGENTS.md` file exists in the workspace root
...
2. **Gather Error Context**:
   ...
   - **If error is from linter**: Use `read_lints` tool to get linter errors
   - **If error is from code execution**: Check recent terminal commands and their outputs
...
2. **After Applying**:
   ...
   - Optionally mention what was changed in brief terms (e.g., "Fixed the type mismatch in UserService.ts")

3. **Verify the Fix**:
   - Check if there are any remaining linter errors
   - If the error was from execution, suggest running the code again to verify
...
2. **Update AGENTS.md**:
   - If AGENTS.md exists and needs updating, add the new information
   - If AGENTS.md doesn't exist but agent state changed, create it with the relevant information
   - Keep updates concise and relevant
   - Maintain the existing format if AGENTS.md already exists
...
- **Check AGENTS.md first**: This saves time by leveraging existing context
...
- **Update documentation**: Keep AGENTS.md current if agent-related information changes
